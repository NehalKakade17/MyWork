...
//					.getResultList();
    ...
    public List<User> getDetailsByRole(UserRole role) {
        ...
        String jpql = "SELECT new com.blogs.pojos.User(u.firstName, u.lastName, u.dob) FROM User u WHERE u.role = :role";  // Updated query
...
//3. Get user from id
	...
	public List<User> getUserNamesByCity(String city1) {
	    ...
	    try {
	        // 3. Create an HQL query to fetch users based on city from Address table
	        String hql = "SELECT u FROM User u JOIN u.userAddress a WHERE a.city = :city";
	        
	        // 4. Execute the query with the city parameter
