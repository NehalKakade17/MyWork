...
public class UserDaoImpl implements UserDao {
	...
	public UserDaoImpl() throws SQLException {
		...
		pst3 = connection.prepareStatement("UPDATE users SET password=? WHERE email=?");// New PreparedStatement for password update
		pst4 = connection.prepareStatement("delete from users where email=?");
	...
	public String registerVoter(User newVoter) throws SQLException {
	  ...
	  public String changePassword(String email, String newPassword) throws SQLException {
	        pst3.setString(1, newPassword); // Set the new password
	        pst3.setString(2, email); // Set the email for the user whose password is to be changed
	  ...
	  public String deleteUser(String email) throws SQLException {
	        pst4.setString(1, email); // Set the email parameter for the user to delete
	  ...
	  public String signIn(String email, String password) throws SQLException {
	        ...
	        try (PreparedStatement pst = connection.prepareStatement(query)) {
	            pst.setString(1, email); // Set email parameter
	            pst.setString(2, password); // Set password parameter
	            
	            try (ResultSet rst = pst.executeQuery()) {
	                // If the user exists and the credentials match
	                if (rst.next()) {
	                    // You can return user details or a success message
	                    return "Sign-in successful! Welcome, " + rst.getString("first_name");
	                } else {
